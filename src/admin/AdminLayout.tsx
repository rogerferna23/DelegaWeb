...
interface NavItem { label: string; icon: LucideIcon; path: string; badge?: string | null; badgeAlert?: boolean }
...
export default function AdminLayout() {
  ...
  const handleReviewInline = async (id: string, approved: boolean) => {
        ...
        <header className="relative z-20 h-12 border-b border-white/5 bg-cardbg/80 backdrop-blur-md flex items-center justify-between px-5 gap-4 flex-shrink-0">
          ...
          <div className="flex items-center gap-2.5">
                  ...
                  <div className="max-h-80 overflow-y-auto">
                    ...
                    {pendingRequests.length > 0 ? (
                      pendingRequests.map(req => {
                        ...
                        const cfg = (ACTION_MAP as Record<string, { label: string; Icon: LucideIcon; color: string; detail: string }>)[req.action] || { label: req.action, Icon: FileText, color: 'bg-white/5 text-gray-400', detail: req.target_email };
            ...
            <div className="flex items-center gap-2">
              ...
              {currentUser?.role === 'superadmin' && (
                <Shield className="w-3 h-3 text-primary" aria-label="Super Admin" />
              )}
            </div>
          </div>
        </header>
