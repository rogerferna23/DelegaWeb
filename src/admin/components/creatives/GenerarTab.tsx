...
type MediaTypeId = 'imagen' | 'video' | 'img2vid' | 'avatar';
...
interface Props {
  onOpenGenerator?: (mediaType: string, preset: CreativePreset) => void;
  onOpenModelSelector?: () => void;
}

export default function GenerarTab({ onOpenGenerator, onOpenModelSelector: _onOpenModelSelector }: Props) {
  const [mediaType, setMediaType] = useState<MediaTypeId>('imagen');
  const [activeCategory, setActiveCategory] = useState('Todos');
  const [searchTerm, setSearchTerm] = useState('');

  const getPresets = (): CreativePreset[] => {
    switch (mediaType) {
      case 'imagen': return IMAGE_PRESETS;
      case 'video': return VIDEO_PRESETS;
      case 'img2vid': return VIDEO_PRESETS;
