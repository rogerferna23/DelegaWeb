...
const renderMessage = (content: unknown) => {
...
interface AIChatPanelProps { isVisible: boolean; setVisible: (v: boolean) => void }

export default function AIChatPanel({ isVisible, setVisible }: AIChatPanelProps) {
  ...
  const [activeModel, setActiveModel] = useState<{ name: string; icon: typeof Sparkles; color?: string }>({ name: 'Cargando...', icon: Sparkles });
  ...
  const fetchActiveModel = async () => {
    try {
      ...
      const modelId = (data?.preferred_claude_model as string) || 'claude-sonnet-4-6';
      const modelMap: Record<string, { name: string; icon: typeof Sparkles; color: string }> = {
  ...
  return createPortal(panelContent, document.body as HTMLElement);
}
