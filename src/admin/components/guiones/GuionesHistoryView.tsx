...
export interface GuionHistorial {
  id: string;
  tipo: string;
  es_optimizado: boolean;
  estructura: string;
  tono: string;
  gancho_preview: string | null;
  favorito: boolean;
  created_at: string;
  user_id: string;
}
...
interface Props {
  onVerDetalle: (guion: GuionHistorial) => void;
}

export default function GuionesHistoryView({ onVerDetalle }: Props) {
  const [guiones, setGuiones] = useState<GuionHistorial[]>([]);
  ...
  const fetchHistory = async () => {
    try {
      ...
      setGuiones((data as GuionHistorial[]) || []);
    } catch (err) {
  ...
  const toggleFavorito = async (id: string, currentFav: boolean) => {
