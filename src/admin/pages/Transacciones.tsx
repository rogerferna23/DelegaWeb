...
export default function Transacciones() {
  ...
  const combinedTransactions = useMemo<Transaction[]>(() => {
    ...
    ventas.forEach(v => {
      all.push({
        ...
        type: 'ingreso',
        date: v.fecha,
        amount: parseFloat(String(v.importe)) || 0,
  ...
  const groupedTransactions = useMemo<MonthGroup[]>(() => {
    ...
    combinedTransactions.forEach(t => {
      ...
      if (!key) return;
      if (!map[key]) map[key] = { key, entries: [], totalIngresos: 0, totalGastos: 0 };
                          ...
                          {entries.map(t => (
                            <tr key={t.id} className="hover:bg-white/[0.02] transition-colors">
                              <td className="px-5 py-2.5 w-10">
                                {t.type === 'ingreso' ? (
                                  <div className="w-6 h-6 rounded-md bg-green-500/10 flex items-center justify-center text-green-400">
