...
export default function Creativos() {
  ...
  const [selectedModel, setSelectedModel] = useState<{ id: string } | null>(null);

  // Sidebar state
  const [showSidebar, setShowSidebar] = useState(false);
  const [activePreset, setActivePreset] = useState<CreativePreset | null>(null);
  const [activeMediaType, setActiveMediaType] = useState('imagen');

  const handleOpenGenerator = (mediaType: string, preset: CreativePreset) => {
  ...
  const handleModelSelected = (model: { id: string }) => {
