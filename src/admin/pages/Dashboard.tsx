  ...
  const { salesData, topProducts, statCards } = useMemo(() => {
    ...
    ventas.forEach(v => {
      if (v.estado === 'pagado') {
        ...
        if (year === currentYear) monthlyRevenue[month] += parseFloat(String(v.importe) || '0');
        if (year === currentYear - 1) lastMonthRevenue[month] += parseFloat(String(v.importe) || '0');

        totalRevenue += parseFloat(String(v.importe) || '0');
      }
    });
        ...
        prodStats.forEach(p => {
          if (v.servicio.toLowerCase().includes(p.name.toLowerCase())) {
            ...
            if (v.servicio === p.name) {
              p.revenue += parseFloat(String(v.importe) || '0');
            } else {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 mb-5">
        {statCards.map(({ label, value, change, icon: IconComponent, positive }) => (
          <div
            key={label}
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={salesData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              ...
              <YAxis 
                ...
                domain={[0, 'auto']}
              />
              <Tooltip content={(props) => <CustomTooltip active={props.active} payload={props.payload as TooltipEntry[] | undefined} label={String(props.label ?? '')} />} cursor={{ stroke: '#f97316', strokeWidth: 1, strokeDasharray: '4 4' }} />
