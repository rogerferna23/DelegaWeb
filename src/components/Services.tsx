...
const Services = () => {
  ...
  const [selectedService, setSelectedService] = useState<ServiceItem | SimpleCartItem | null>(null);

  const parsePrice = (priceStr: string): number =>
    parseInt(priceStr.replace(/[^0-9]/g, ''), 10);

  const handleAddToCart = (service: ServiceItem | SimpleCartItem) => {
    addToCart({
      name: service.title,
  ...
  const isVariablePrice = (s: ServiceItem) => s.price.includes('–') || s.price.includes('-');
        ...
        {servicesList.filter(s => !s.premium).map((service, index) => (
          ...
          >
            ...
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center
              ...
              {cloneElement(service.icon, { className: 'w-5 h-5 text-primary' })}
            </div>
          ...
          <div className="flex items-start justify-between gap-4 mb-2">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-500/30 to-amber-600/10 border border-amber-500/20 flex items-center justify-center shadow-inner shadow-amber-500/10">
                {cloneElement(service.icon, { className: 'w-6 h-6 text-amber-400' })}
              </div>
              <div>
