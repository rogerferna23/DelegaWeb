...
export default function PayPalCheckout({ cartItems, cartTotal, onClose, onSuccess }) {
          ...
          {status === 'success' && (
            <div className="text-center py-4">
              ...
              <p className="text-gray-400 text-xs mb-3">
                Hola {payer?.name?.given_name}, recibirás un email de confirmación en{' '}
                <span className="text-white">{payer?.email_address}</span>.
              </p>
              ...
              {dbSyncError && (
                <div className="text-left bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 mb-4">
                  ...
                  <p className="text-[10px] text-amber-200/90 font-semibold bg-black/20 p-2 rounded-lg break-all">
                    ...
                    <span className="text-white font-mono text-xs mt-1 block">{orderDetails?.id}</span>
                  </p>
