...
const Features = () => {
  const featuresList = [
    ...
    },
    {
      ...
      icon: <Lightbulb className="text-primary w-5 h-5" />
    },
    {
      title: "Soporte personalizado",
      icon: <Headphones className="text-primary w-5 h-5" />
    }
  ];
    ...
    <section className="w-full py-16 max-w-5xl mx-auto border-t border-gray-800/30">
      ...
      <div className="flex flex-wrap justify-center gap-4">
        {featuresList.map((feature, index) => (
          <div key={index} className="bg-cardbg border border-gray-800/60 rounded-xl p-4 flex items-center gap-3 hover:border-gray-700 transition-colors min-w-[250px]">
            <div className="w-8 h-8 rounded-lg bg-gray-900/50 flex items-center justify-center shrink-0">
              {React.cloneElement(feature.icon, { className: "text-primary w-4 h-4" })}
            </div>
